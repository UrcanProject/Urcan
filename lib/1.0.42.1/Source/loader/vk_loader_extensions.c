...
VKAPI_ATTR void VKAPI_CALL loader_init_instance_extension_dispatch_table(VkLayerInstanceDispatchTable *table, PFN_vkGetInstanceProcAddr gpa,
    ...
    table->GetPhysicalDeviceProperties2KHX = (PFN_vkGetPhysicalDeviceProperties2KHX)gpa(inst, "vkGetPhysicalDeviceProperties2KHX");
    table->GetPhysicalDeviceImageFormatProperties2KHX = (PFN_vkGetPhysicalDeviceImageFormatProperties2KHX)gpa(inst, "vkGetPhysicalDeviceImageFormatProperties2KHX");
...
VKAPI_ATTR void* VKAPI_CALL loader_lookup_instance_dispatch_table(const VkLayerInstanceDispatchTable *table, const char *name,
    ...
    *found_name = true;
    ...
    if (!strcmp(name, "GetPhysicalDeviceProperties2KHX")) return (void *)table->GetPhysicalDeviceProperties2KHX;
    if (!strcmp(name, "GetPhysicalDeviceImageFormatProperties2KHX")) return (void *)table->GetPhysicalDeviceImageFormatProperties2KHX;
...
VKAPI_ATTR void VKAPI_CALL terminator_GetPhysicalDeviceProperties2KHX(
    ...
    struct loader_physical_device_term *phys_dev_term = (struct loader_physical_device_term *)physicalDevice;
    struct loader_icd_term *icd_term = phys_dev_term->this_icd_term;
    if (NULL == icd_term->dispatch.GetPhysicalDeviceProperties2KHX) {
        loader_log(icd_term->this_instance, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                   "ICD associated with VkPhysicalDevice does not support GetPhysicalDeviceProperties2KHX");
    }
    icd_term->dispatch.GetPhysicalDeviceProperties2KHX(phys_dev_term->phys_dev, pProperties);
}
...
VKAPI_ATTR VkResult VKAPI_CALL vkGetPhysicalDeviceImageFormatProperties2KHX(
    ...
    VkPhysicalDevice unwrapped_phys_dev = loader_unwrap_physical_device(physicalDevice);
    disp = loader_get_instance_layer_dispatch(physicalDevice);
    return disp->GetPhysicalDeviceImageFormatProperties2KHX(unwrapped_phys_dev, pImageFormatInfo, pImageFormatProperties);
}

VKAPI_ATTR VkResult VKAPI_CALL terminator_GetPhysicalDeviceImageFormatProperties2KHX(
    VkPhysicalDevice                            physicalDevice,
    const VkPhysicalDeviceImageFormatInfo2KHX*  pImageFormatInfo,
    VkImageFormatProperties2KHX*                pImageFormatProperties) {
    struct loader_physical_device_term *phys_dev_term = (struct loader_physical_device_term *)physicalDevice;
    struct loader_icd_term *icd_term = phys_dev_term->this_icd_term;
    if (NULL == icd_term->dispatch.GetPhysicalDeviceImageFormatProperties2KHX) {
        loader_log(icd_term->this_instance, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                   "ICD associated with VkPhysicalDevice does not support GetPhysicalDeviceImageFormatProperties2KHX");
    }
    return icd_term->dispatch.GetPhysicalDeviceImageFormatProperties2KHX(phys_dev_term->phys_dev, pImageFormatInfo, pImageFormatProperties);
}
...
VKAPI_ATTR VkResult VKAPI_CALL vkEnumeratePhysicalDeviceGroupsKHX(
    ...
    for (uint32_t group = 0; group < *pPhysicalDeviceGroupCount; group++) {
        for (uint32_t dev = 0; dev < pPhysicalDeviceGroupProperties[group].physicalDeviceCount; dev++) {
...
VKAPI_ATTR VkResult VKAPI_CALL terminator_EnumeratePhysicalDeviceGroupsKHX(
    ...
    while (NULL != icd_term) {
        if (NULL != icd_term->dispatch.EnumeratePhysicalDeviceGroupsKHX) {
            ...
            res = icd_term->dispatch.EnumeratePhysicalDeviceGroupsKHX(
                icd_term->instance, &cur_group_count, NULL);
            if (res != VK_SUCCESS) {
                break;
            } else if (NULL != pPhysicalDeviceGroupProperties && max_group_count > total_group_count) {
            ...
            // For ICDs which don't directly support this, create a group for each physical device
            for (uint32_t j = 0; j < inst->total_gpu_count; j++) {
                if (inst->phys_devs_term[j]->icd_index == i) {
                    if (NULL != pPhysicalDeviceGroupProperties && max_group_count > total_group_count) {
                        pPhysicalDeviceGroupProperties[total_group_count].physicalDeviceCount = 1;
                        pPhysicalDeviceGroupProperties[total_group_count].physicalDevices[0] =
    ...
    *pPhysicalDeviceGroupCount = total_group_count;
    ...
    if (NULL != pPhysicalDeviceGroupProperties) {
        for (uint32_t group = 0; group < max_group_count; group++) {
            VkPhysicalDeviceGroupPropertiesKHX *cur_props = &pPhysicalDeviceGroupProperties[group];
            for (i = 0; i < cur_props->physicalDeviceCount; i++) {
                for (uint32_t term = 0; term < inst->total_gpu_count; term++) {
                    if (inst->phys_devs_term[term]->phys_dev == cur_props->physicalDevices[i]) {
                        cur_props->physicalDevices[i] = (VkPhysicalDevice)inst->phys_devs_term[term];
                    }
                }
            }
        }
...
bool extension_instance_gpa(struct loader_instance *ptr_instance, const char *name, void **addr) {
    *addr = NULL;
    ...
    if (!strcmp("vkCmdPushDescriptorSetKHR", name)) {
        *addr = (void *)vkCmdPushDescriptorSetKHR;
        return true;
    }

    // ---- VK_KHR_descriptor_update_template extension commands
    if (!strcmp("vkCreateDescriptorUpdateTemplateKHR", name)) {
        *addr = (void *)vkCreateDescriptorUpdateTemplateKHR;
        return true;
    }
    if (!strcmp("vkDestroyDescriptorUpdateTemplateKHR", name)) {
        *addr = (void *)vkDestroyDescriptorUpdateTemplateKHR;
        return true;
    }
    if (!strcmp("vkUpdateDescriptorSetWithTemplateKHR", name)) {
        *addr = (void *)vkUpdateDescriptorSetWithTemplateKHR;
        return true;
    }
    if (!strcmp("vkCmdPushDescriptorSetWithTemplateKHR", name)) {
        *addr = (void *)vkCmdPushDescriptorSetWithTemplateKHR;
        return true;
    }
    ...
    if (!strcmp("vkCmdSetViewportWScalingNV", name)) {
        *addr = (void *)vkCmdSetViewportWScalingNV;
        return true;
    }
    ...
    if (!strcmp("vkDisplayPowerControlEXT", name)) {
        *addr = (void *)vkDisplayPowerControlEXT;
        return true;
    }
    if (!strcmp("vkRegisterDeviceEventEXT", name)) {
        *addr = (void *)vkRegisterDeviceEventEXT;
        return true;
    }
    if (!strcmp("vkRegisterDisplayEventEXT", name)) {
        *addr = (void *)vkRegisterDisplayEventEXT;
        return true;
    }
    if (!strcmp("vkGetSwapchainCounterEXT", name)) {
        *addr = (void *)vkGetSwapchainCounterEXT;
        return true;
    }

    // ---- VK_EXT_discard_rectangles extension commands
    if (!strcmp("vkCmdSetDiscardRectangleEXT", name)) {
        *addr = (void *)vkCmdSetDiscardRectangleEXT;
        return true;
    }
...
const VkLayerInstanceDispatchTable instance_disp = {
    ...
    .GetPhysicalDeviceProperties2KHX = terminator_GetPhysicalDeviceProperties2KHX,
    .GetPhysicalDeviceImageFormatProperties2KHX = terminator_GetPhysicalDeviceImageFormatProperties2KHX,
