    ...
    const VkWriteDescriptorSet*                 pDescriptorWrites)
{
    ...
    dev_data->device_dispatch_table->CmdPushDescriptorSetKHR(commandBuffer, pipelineBindPoint, layout, set, descriptorWriteCount, (const VkWriteDescriptorSet*)local_pDescriptorWrites);
    if (local_pDescriptorWrites)
        delete[] local_pDescriptorWrites;
}
    ...
    VkRefreshCycleDurationGOOGLE*               pDisplayTimingProperties)
{
    ...
    VkResult result = dev_data->device_dispatch_table->GetRefreshCycleDurationGOOGLE(device, swapchain, pDisplayTimingProperties);
    ...
    VkPastPresentationTimingGOOGLE*             pPresentationTimings)
{
    ...
    VkResult result = dev_data->device_dispatch_table->GetPastPresentationTimingGOOGLE(device, swapchain, pPresentationTimingCount, pPresentationTimings);
    ...
    VkSwapchainKHR *local_pSwapchains = NULL;
    {
        ...
        if (pSwapchains) {
            ...
            for (uint32_t index0 = 0; index0 < swapchainCount; ++index0) {
                local_pSwapchains[index0] = (VkSwapchainKHR)dev_data->unique_id_mapping[reinterpret_cast<const uint64_t &>(pSwapchains[index0])];
            }
        }
    }
    dev_data->device_dispatch_table->SetHdrMetadataEXT(device, swapchainCount, (const VkSwapchainKHR*)local_pSwapchains, pMetadata);
