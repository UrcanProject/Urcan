	...
	for (uint32_t z = 0; z < heights.size(); z++)
		for (uint32_t x = 0; x  < heights[0].size(); x++) {
			for (uint32_t i = 0; i < verticesMod.size(); i++)
				this->_vertices[(x + z * heights[z].size()) * verticesMod.size() + i] = {combineVec3(i, x + startX, z + startZ, heights[z][x], norm), verticesMod[i].color};
			for (uint32_t i = 0; i < indicesMod.size(); i++)
				this->_indexes[(x + z * heights[z].size()) * indicesMod.size() + i] = ((x + z * heights[z].size()) * verticesMod.size() + indicesMod[i]);
		}
}
